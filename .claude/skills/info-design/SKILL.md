...
| 항목 | 규정 |
|------|------|
| 프로젝트 유형 판정 | 코드 생성 전 `references/project-profiles.md` 기준으로 민간/사내/CMS/공공기관/정부 상징 사용 여부를 먼저 판정 |
...
| 작업 맥락 | 로드 |
|-----------|------|
| 코드 생성 전 프로젝트 유형 판정 | `references/project-profiles.md` |
...
1. ✅ 사이트 유형을 `private-corporate`, `admin-cms`, `public-affiliated`, `public-government`, `ecommerce` 중 하나로 판정했는가?
...
- 프로젝트가 민간/사내/CMS/공공기관/정부 상징 사용 서비스 중 무엇인지 모호
...
3. **프로젝트 프로파일** — 정부 아이덴티티 요소는 적용 조건이 확인된 경우만
