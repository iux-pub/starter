...
현재 확인된 첫 검토 후보는 `/Users/johyeonchang/Documents/Work/code/gitCode/INFO_INSTACK_PORTAL_WORK`다. 단, 이 문서는 후보를 추적하기 위한 참고이며 해당 개발팀 레포에 규칙을 즉시 적용한다는 뜻이 아니다. 개발팀 코드 수정, `CLAUDE.md` 변경, lint/type/build 실행, 토큰 CSS 주입은 UX팀장 승인 후 별도 작업으로만 진행한다.
...
적용 전에는 해당 프로젝트의 기존 `CLAUDE.md`와 개발팀 운영 규칙을 먼저 읽는다. 프로젝트 고유 규칙(API 호출 방식, 패키지 매니저, 금지된 검증 명령, 포트, 라우팅 구조 등)은 보존하고, info-design은 UI/스타일/마크업 범위의 제안 또는 승인된 컨트랙트로만 병합한다.
